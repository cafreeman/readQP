...
test_that("readModelFile can read a CPLEX file successfully", {
  test_path <- getSampleData("simple_qp", TRUE)
  file <- readModelFile(test_path)
  expect_equal(file, "Minimize\nobj: a + b + [ a^2 + 4 a * b + 7 b^2 ]/2\nSubject To\nc1: a + b >= 10\nEnd\n")
})

test_that("readModelFile errors when the user doesn't point to a CPLEX file", {
...
test_that("readQP can correctly turn a QP model file into a model object", {
  test_path <- getSampleData("qp_3_terms", TRUE)
  model <- readQP(test_path)
...
test_that("readQP can handle in an insane LP file and return the correct object", {
  ...
  expect_message(model <- readQP(test_path), "No quadratic elements detected. Defaulting to Rglpk_read_file.")
